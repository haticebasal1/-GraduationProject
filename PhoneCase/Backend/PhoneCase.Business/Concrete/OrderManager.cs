...
public class OrderManager : IOrderService
{
    ...
    {
        {
            try
            {
                ...
                if (order is null)
                {
                    return ResponseDto<NoContentDto>.Fail("Sipariş bulunamadığıb için iptal edilemedi!", StatusCodes.Status404NotFound);
                }
    ...
    public async Task<ResponseDto<OrderDto>> OrderNowAsync(OrderNowDto orderNowDto)
    {
        try
        {
            ...
            {
                return ResponseDto<OrderDto>.Fail($"{string.Join(",", orderNowDto.OrderItems.Select(x => x.ProductId))} id'li ürün/ürünler veri tabanında bulunmadığı için sipariş işlemi tamamlanamadı!", StatusCodes.Status404NotFound);
            }
